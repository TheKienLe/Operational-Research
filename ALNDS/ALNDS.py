...
class Parameters(): 
    ...
    # hub_data = ["Hub", "A:F", 0, 6]  
    # distance_data = ["Distance", "A:AT", 0, 16]  
    # demand_data = ["Demand_DCs", "F", 0, 10]
    # Q_vehicle = 120000   
    # always_open_hubs = [1]  
    file_path = "./data/real_data.xlsx"  
...
class Problem():
    ...
    def __init__(self, file_path: str) -> None:  
        ...
        self.demand = np.array(self.demand, dtype=np.float64).flatten()

        self.number_of_non_hub = len(self.demand)  
        self.non_hubs = []  
...
linked_nodes)  
                        ...
                        current_delivery_capacity =  current_delivery_capacity - current_node_v.D
                        if current_delivery_capacity > 0 :
...
class CostFunction():
    ...
    def caculate_cost_hub(hub, problem, final=False, detail=False):
        ...
        for route in hub.routes:
            for node in route:
                total_demand_load += node.demand  # Sum the demand for delivery nodes
        ...
        for route in hub.routes:
            ...
            for k in range(len(route_id) - 1):
                ...
                if j in node_visited:
                    ...
                    node_visited.append(j)
                total_quantity_load += demand[j-7]  # Update the calculation for delivery only
