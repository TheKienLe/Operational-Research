import numpy as np
import copy
from vrp_base import VrptwGraph
from threading import Event

class Ant:
    def __init__(self, graph: VrptwGraph, start_index=0):
        """
        Initialize an Ant instance for the VRPTW problem.

        Parameters:
        graph (VrptwGraph): The VRPTW graph.
        start_index (int): The starting index (default is 0).
        """
        super()
        self.graph = graph
    ...
    def clear(self):
        """Clear the travel path and index to visit."""
        self.travel_path.clear()
        self.index_to_visit.clear()

    def move_to_next_index(self, next_index):
        """
        Move the ant to the next index and update its state.

        Parameters:
        next_index (int): The next index to visit.
        """
        # Update travel path and total travel distance
        ...
        if self.graph.nodes[next_index].is_depot:
            # If the next position is a depot, reset vehicle load and travel time
            self.vehicle_load = 0
            self.vehicle_travel_time = 0
        else:
            # Update vehicle load, travel time, and time window constraints
    ...
    def index_to_visit_empty(self):
        ...
        Returns:
        bool: True if no more nodes to visit, False otherwise.
        """
        return len(self.index_to_visit) == 0

    def get_active_vehicles_num(self):
        """
        Get the number of active vehicles used.

        Returns:
        int: Number of active vehicles.
        """
        return self.travel_path.count(0) - 1

    def check_condition(self, next_index) -> bool:
        """
        Check if moving to the next index meets the constraints.

        Parameters:
        next_index (int): The next index to check.
        ...
        # Check if the vehicle can return to the depot on time
        ...
        # Check if the vehicle can reach the next node within its due time
    ...
    def cal_next_index_meet_constrains(self):
        """
        Find all customers that can be reached from the current index.

        Returns:
        list: List of indices that meet the constraints.
        """
    ...
    def cal_nearest_next_index(self, next_index_list):
        """
        Find the nearest customer from the current index.

        Parameters:
        next_index_list (list): List of candidate indices.

        Returns:
        int: The index of the nearest customer.
        """
    ...
    def cal_total_travel_distance(graph: VrptwGraph, travel_path):
        ...
        Parameters:
        graph (VrptwGraph): The VRPTW graph.
        travel_path (list): The travel path.
    ...
    def try_insert_on_path(self, node_id, stop_event: Event):
        """
        Try to insert a node into the current travel path without violating constraints.

        Parameters:
        node_id (int): The node ID to insert.
        stop_event (Event): Stop event for terminating the process.

        Returns:
        int: The best insert index or None if no valid insertion point is found.
        """
    ...
    def insertion_procedure(self, stop_event: Event):
        """
        Insert all unvisited nodes into the current travel path without violating constraints.
    ...
    def local_search_once(graph: VrptwGraph, travel_path: list, travel_distance: float, i_start, stop_event: Event):
        """
        Perform one iteration of local search using cross-exchange.

        Parameters:
        graph (VrptwGraph): The VRPTW graph.
        travel_path (list): The travel path.
        travel_distance (float): The travel distance.
        i_start (int): The start index for the local search.
        stop_event (Event): Stop event for terminating the process.

        Returns:
        tuple: New path, new path distance, and new start index or (None, None, None) if no improvement.
        """
        depot_ind = [ind for ind in range(len(travel_path)) if graph.nodes[travel_path[ind]].is_depot]
    ...
    def local_search_procedure(self, stop_event: Event):
        """
        Perform local search on the current travel path using cross-exchange.
